...
def replacing_trace_zero(trace, activeTiles, lam, gamma):
    '''To do: replacing trace update rule
    # trace: old trace 
    # activeTiles: current active tile indices
    # lam: lambda
...
class SARSA_lambda_Learner(object):
    def __init__(self, env, traceUpdate,traceUpdate_zero):
        ...
        self.traceUpdate_zero = traceUpdate_zero
...
#%%
    def learn(self, obs, action, reward, done, next_obs, next_action):
        ...
        td_delta = reward + self.gamma*(not done)*np.max(self.Q[discretized_next_sa[0],discretized_next_sa[1],:])-self.Q[discretized_sa[0],discretized_sa[1],discretized_sa[2]]
        # Update the visit counts as statistics for later analysis.
        self.visit_counts[discretized_obs][action] += 1
        # Update the trace
        if next_action==np.max(self.Q[discretized_next_sa[0],discretized_next_sa[1],:]):
            self.traceUpdate(self.trace, discretized_sa, self.lam, self.gamma)
        else:
            self.traceUpdate_zero(self.trace, discretized_sa, self.lam, self.gamma)
...
if __name__ == "__main__":
    ...
    agent = SARSA_lambda_Learner(env, replacing_trace,replacing_trace_zero)
    learned_policy, Q, visit_counts, episodic_returns = train(agent, env, MAX_NUM_EPISODES)
    # save the Q-table here
    np.save('WATKINS_' + 'RIC_' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'.npy', Q)
    # save episodic returns
    np.save('WATKINS_REWARDS_' + 'RIC_' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'.npy',episodic_returns)

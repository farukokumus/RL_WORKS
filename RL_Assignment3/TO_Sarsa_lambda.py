...
#%%
    def learn(self, obs, action, reward, done, next_obs,next_action):
        ...
        delta= self.Q[discretized_sa]-self.Qold  
        self.Qold=self.Q[discretized_next_sa]        
        ...
        # Update the trace
        self.traceUpdate(self.trace, discretized_sa, self.lam, self.gamma,self.alpha)
        
        # To Do: update the q table
        self.Q+=self.alpha*(td_delta+delta)*self.trace
        self.Q[discretized_sa]-=self.alpha*delta
        #self.Q-=self.alpha*delta
...
if __name__ == "__main__":
    ...
    np.save('TO_SARSA_' + 'FARUK_' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'.npy', Q)
    # save episodic returns
    #print(episodic_returns)
    np.save('TO_SARSA_REWARDS_' + 'FARUK_' + str(agent.lam) + '_' + str(MAX_NUM_EPISODES) +'.npy',episodic_returns)

...
Original file is located at
    https://colab.research.google.com/drive/13QB-LDRz5yj9CPTjoQXkDcnS6Fel_AOo
"""
...
class QNetwork(nn.Module):    
    def __init__(self, input_shape, num_actions):
        ...
        self.fc1 = nn.Linear(in_features=64*7*7, out_features=256) # buradaki input size ı değiştitp bi bak self.fc_input_dim 64*7*7
...
class LearningAgent: 
    ...
    def update(self, batch_size):       
        ...
        with torch.no_grad():
          #DDQN
          max_act_idx = self.q_net1(next_states).argmax(1)
          labels_next = self.target_q_net1(next_states).gather(1, max_act_idx.unsqueeze(1))
          labels = rewards + (self.gamma* labels_next.squeeze(1)*(1-dones))        
        predicted_targets = self.q_net1(states).gather(1, actions.unsqueeze(1)) #self.q_net1(states).gather(1, act_idx)


        # To do:  q loss using self.criterion
        q1_loss = self.criterion(predicted_targets, labels.detach().unsqueeze(1)).squeeze()     
...
class SumTree:
    ...
    def min_prob(self):
        ...
        if self.overwrite_start_flag == False:        
            ...
            p_min = min(np.min(p),self.p_min_history)
...
class PER(object):
    ...
    def sample(self, batch_size):
        ...
        min_p=self.sum_tree.min_prob()
        min_p = min_p + 1e-7 if min_p == 0 else min_p
        #min_p/=sum(priorities)
        ...
        max_weight = np.power((self.current_length * min_p), -self.beta) # Hint : you will need to call self.sum_tree.min_prob()

...
#%% update Q table 
    def update_Q_table(self, obs, action, reward, done, next_obs):
        ...
        td_target = reward + self.gamma *(not done)*np.max(self.Q[discretized_next_obs])
        td_error = td_target - self.Q[discretized_obs][action]
        self.Q[discretized_obs][action] += self.alpha * td_error # 1/self.visit_counts[discretized_obs[0],discretized_obs[1],action] / self.alpha 
...
def train(agent, env, MAX_NUM_EPISODES):
    ...
    for episode in range(MAX_NUM_EPISODES):
        ...
        while not done:
            ...
            disc_obs=agent.discretize(obs)
            next_obs, reward, done, info = env.step(action) #2
            agent.update_Q_table(obs, action, reward,done, next_obs) #3 prob
            obs = next_obs
            episodic_return += reward #4
            agent.visit_counts[disc_obs[0],disc_obs[1],action]+=1 #5
...
if __name__ == "__main__":
    ...
    #plot
    np.save("faruk_qlearning_episodic_returns.npy",episodic_returns)
    y= np.load("faruk_qlearning_episodic_returns.npy")
    x=np.arange(2000)
    plt.plot(x,y)
    plt.show()
